    ...
    const handlePermanentDelete = async (clinic) => {
        if (window.confirm(`⚠️ تحذير: هل أنت متأكد من حذف عيادة "${clinic.clinic_name}" نهائياً؟ هذا سيؤدي لحذف العيادة وجميع بيانات المرضى وحساب الطبيب المرتبط بها.`)) {
            try {
                ...
                if (!session) { toast.error('انتهت جلستك، أعد تسجيل الدخول.'); return; }
    ...
    return (
        <div className="space-y-6">
            ...
            <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <div>
                    <h1 className="text-xl md:text-2xl font-bold">إدارة العيادات</h1>
                    <p className="text-slate-400 text-sm">إضافة وتعديل اشتراكات العيادات</p>
                </div>
                <button
                    onClick={() => setIsModalOpen(true)}
                    className="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-purple-900/20 self-start sm:self-auto"
                >
                    <Plus size={18} />
                    <span className="text-sm">إضافة عيادة</span>
                </button>
            </header>
            ...
            {/* Clinics Table - horizontal scroll on mobile */}
            <div className="glass-panel overflow-hidden rounded-2xl border border-white/5">
                <div className="overflow-x-auto">
                    <table className="w-full text-right min-w-[600px]">
                        <thead className="bg-white/5 text-slate-400">
                            <tr>
                                <th className="p-3 md:p-4 font-medium">اسم العيادة</th>
                                <th className="p-3 md:p-4 font-medium hidden sm:table-cell">التخصص</th>
                                <th className="p-3 md:p-4 font-medium">الحالة</th>
                                <th className="p-3 md:p-4 font-medium hidden md:table-cell">تاريخ الاشتراك</th>
                                <th className="p-3 md:p-4 font-medium">الإجراءات</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                ...
                                filteredClinics.map((clinic) => (
                                    <tr key={clinic.id} className="hover:bg-white/5 transition-colors border-b border-white/5 last:border-0">
                                        <td className="p-3 md:p-4 font-bold max-w-[160px]">
                                            <span className="block truncate">{clinic.clinic_name}</span>
                                        </td>
                                        <td className="p-3 md:p-4 text-slate-300 hidden sm:table-cell">{clinic.specialty}</td>
                                        <td className="p-3 md:p-4">
                                            <span className={`px-2 py-1 rounded text-xs font-bold border whitespace-nowrap ${clinic.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : clinic.status === 'suspended' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'}`}>
                                                {clinic.status === 'active' ? 'نشط' : clinic.status === 'suspended' ? 'موقوف' : 'غير نشط'}
                                            </span>
                                        </td>
                                        <td className="p-3 md:p-4 text-slate-400 hidden md:table-cell">
                                        ...
                                        <td className="p-3 md:p-4">
                                            <div className="flex gap-1 md:gap-2">
                                                <button
                                                    ...
                                                    className="p-1.5 md:p-2 hover:bg-slate-700 rounded-lg text-emerald-400"
                                                ...
                                                <button
                                                    ...
                                                    className="p-1.5 md:p-2 hover:bg-slate-700 rounded-lg text-sky-400"
                                                ...
                                                <button
                                                    ...
                                                    className="p-1.5 md:p-2 hover:bg-slate-700 rounded-lg text-yellow-400 transition-colors"
                                                    title="إيقاف العيادة"
                                                >
                                                    <PowerOff size={16} />
                                                </button>
                                                <button
                                                    ...
                                                    className="p-1.5 md:p-2 hover:bg-slate-700 rounded-lg text-red-500 transition-colors"
                                                    title="حذف نهائي"
                                                >
                                                    <Trash2 size={16} />
                                                </button>
